 ...
 - ${executableDirMessage}`;
  ...
  _parseCommand(operands, unknown) {
    ...
    const checkForUnknownOptions = () => {
      if (!this._allowUnknownOption && unknown.length > 0) {
    ...
    if (this.parent && this.parent.listenerCount(commandEvent)) {
      ...
      if (this.listenerCount('command:*')) {
        ...
        if (!isFlag(operands[0])) {
          this.unknownCommand(operands[0]);
        } else {
          // Display help instead of a confusing unknown command error message.
          // The line is only ever reached when allowUnknownOption is on.
          this.help({ error: true });
        }
      } else {
        checkForUnknownOptions();
  ...
  parseOptions(argv) {
    ...
    let treatUnknownOptionAsCommandArgument;
    let onlyConsumeHelpOption = false;
    while (args.length) {
      ...
      if (onlyKnownOptionsSoFar) {
        ...
        treatUnknownOptionAsCommandArgument = (
      ...
      const isArgUnknownOptionTreatedAsSuch = (
        isArgFlag && !treatUnknownOptionAsCommandArgument
      );
      if (isArgUnknownOptionTreatedAsSuch) {
        // The unknown option and all the arguments following it are to be reprocessed by a subcommand.
        dest = unknown;
      }
